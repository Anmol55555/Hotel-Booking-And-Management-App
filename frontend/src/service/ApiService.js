import axios from "axios"           // axios, a js library is used to make HTTP request and to send data to backend server endpoints
                                    // like axios.post will make a POST request
export default class ApiService {
    ...
    static async registerUser(registration) {           // async makes the fnuction asynchronous and enable the use of await keyword
        const response = await axios.post(`${this.BASE_URL}/auth/register`, registration)    // This await will return a promise
        return response.data                            // await will pause the execution of this registerUser() function till the returened promise is resolved, but since the function is async, all the code outside this registerUser() will keep on executing
    }
    ...
    // will return true, if the user has token not null
    static isAuthenticated() {
        const token = localStorage.getItem('token')
        return !!token      // double !! converts value to boolean
    }
